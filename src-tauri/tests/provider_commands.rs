...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".puppyrouter-app").join("settings.json")
}
...
"#
    ...
    assert!(
        auth_value
            ...
            .and_then(|v| v.as_str())
            .is_none(),
        "Codex provider switching should not preserve raw OPENAI_API_KEY as official login state"
    );
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let db_path = std::path::Path::new(&home_dir)
        ...
        .join("puppyrouter-app.db");
    assert!(
        db_path.exists(),
        "switching provider should persist to puppyrouter-app.db"
    );
