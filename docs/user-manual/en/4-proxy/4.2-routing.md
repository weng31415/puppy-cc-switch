...
App routing means letting puppyrouter app route a specific application's API requests through the local routing service.
...
When routing is enabled, puppyrouter app modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, puppyrouter app will:
...
Before enabling routing, puppyrouter app backs up the original configuration and restores it when disabled.
