...
puppyrouter app for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/puppyrouter-app/releases).
...
2. Add puppyrouter app to your antivirus software's whitelist
...
chmod +x puppyrouter-app-*.AppImage

# If it still fails, try
./puppyrouter-app-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.puppyrouter-app/` directory exists
2. Restore from backup: `~/.puppyrouter-app/backups/`
...
**Solutions**:
1. Confirm the file was exported by puppyrouter app
...
**Windows**:
- Check taskbar settings to ensure the puppyrouter app icon is not hidden
...
2. Restart the app
3. Delete `~/.puppyrouter-app/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask puppyrouter-app`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and puppyrouter app runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, puppyrouter app covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `puppyrouter://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/farion1231/puppyrouter-app/issues)
2. Search for similar issues
3. If none found, create a new Issue
4. Provide the following information:
   - Operating system and version
   - puppyrouter app version
...
- macOS/Linux: `~/.puppyrouter-app/logs/`
- Windows: `%APPDATA%\puppyrouter-app\logs\`
