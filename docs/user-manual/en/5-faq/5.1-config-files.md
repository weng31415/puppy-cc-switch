...
## puppyrouter app Data Storage

### Storage Directory

Default location: `~/.puppyrouter-app/`
...
```
~/.puppyrouter-app/
├── puppyrouter-app.db      # SQLite database (SSOT)
...
`puppyrouter-app.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. puppyrouter app writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to puppyrouter app's "providers") |
...
puppyrouter app's priority when modifying configurations:

1. **puppyrouter app Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by puppyrouter app)
- puppyrouter app's `settings.json`

### Not Recommended to Edit Manually

- `puppyrouter-app.db` database file
- Backup files
...
puppyrouter app v3.7.0 migrated from JSON files to SQLite:
