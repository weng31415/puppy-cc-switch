...
This section describes how to configure puppyrouter app according to your preferences.
...
puppyrouter app supports four languages:
...
When enabled, puppyrouter app automatically runs when the system starts.
...
Starting from v3.13.0, puppyrouter app adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger puppyrouter app via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If puppyrouter app runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, puppyrouter app automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that puppyrouter app uses when opening a terminal.
...
The storage location for puppyrouter app's own data, defaulting to `~/.puppyrouter-app/`.
...
The exported file name format is `puppyrouter-app-export-{timestamp}.sql`.
...
Configure puppyrouter app's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, puppyrouter app automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `puppyrouter-app-sync`) |
...
- puppyrouter app automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, puppyrouter app displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current puppyrouter app version number, with support for:
